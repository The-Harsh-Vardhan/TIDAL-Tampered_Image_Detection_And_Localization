...
Directly inform the project's problem framing, architecture, evaluation metrics, and robustness testing.
...
2. **Comprehensive Evaluation of Image Forgery Detection Methods**
   ...
   Survey compiling datasets, metrics (F1, IoU, AUC-ROC), and evaluation methodology.
...
6. **Multistream Identity-Document Image Tampering Detection**
   `043018_1.pdf`
   Multi-stream fusion; useful for future multi-domain architecture design.

7. **Enhanced Image Tampering Detection Using ELA and CNN**
   `ETASR_9593.pdf`
   ELA-based classification; supports optional ELA channel (Phase 2). 96.21% on CASIA v2.0.
...
8. **Multi-Scale Weber Local Descriptor Evaluation**
   ...
   Classical feature approach; historical context for handcrafted features.

9. **Copy-Move Detection with Evolving Circular Domains**
   `s11042-022-12755-w.pdf`
   Category-specific; background for copy-move subset evaluation.

10. **Hybrid Deep Forgery Detection Model**
    `s11042-023-15475-x.pdf`
    Heavier hybrid architecture; demonstrates stronger models beyond the MVP baseline.
...
- `IJCRT24A5072.pdf` — Basic CNN classification (too shallow for segmentation baseline)
- `deep-learning-based-image-tamper-detection-techniques-a-study-IJERTV13IS020023.pdf` — Generic review
- `A_Review_on_Video_Image_Authentication_a.pdf` — Historical review
- `IMAGE_TAMPERING_DETECTION_A_REVIEW_OF_MULTI-TECHNI.pdf` — Broad review

---

## Reference Kaggle Notebooks

- **image-detection-with-mask.ipynb** — UNet baseline for tamper detection with segmentation masks. Validates U-Net + BCE + Dice approach and pixel-level localization framing.
...
| Albumentations | ≥ 1.3.1, < 2.0 | Training augmentations and degradation transforms |
| OpenCV (`cv2`) | ≥ 4.x | Image I/O and preprocessing |
| Weights & Biases (`wandb`) | ≥ 0.15 | Optional experiment tracking |
| Kaggle Secrets API | — | Secure API key retrieval (Kaggle) |
| Colab Secrets (`userdata`) | — | Secure API key retrieval (Colab) |
| opendatasets | ≥ 0.1 | Fallback Kaggle dataset download (Colab) |
| kaggle | ≥ 1.6, < 1.7 | Kaggle API for dataset download (Colab) |
