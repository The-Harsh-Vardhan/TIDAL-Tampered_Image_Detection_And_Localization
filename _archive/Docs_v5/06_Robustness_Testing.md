...
**Research context:** Post-processing robustness testing is a standard practice in image forensics research. Papers P4 (FENet), P13 (EMT-Net), and P17 (ME-Net) all include systematic degradation evaluation. The degradation categories below align with those used in the research literature. JPEG compression is particularly important because social media platforms re-encode images at QF 70–85, which can destroy subtle splicing boundaries.
...
- **JPEG compression** is the most common real-world degradation. Social media platforms typically re-encode at QF 70–85, which can destroy subtle splicing boundaries. Research paper P13 (EMT-Net) demonstrates robustness to JPEG quality as low as 50.
...
- **Blur** destroys high-frequency forensic cues that segmentation models may rely on. Research paper P13 shows robustness to Gaussian blur up to kernel size 15.
- **Resize** causes interpolation artifacts that compete with the tampering artifacts the model was trained to detect.

These degradation types are consistent with the robustness evaluation protocols in the research literature (P4, P13, P17).
