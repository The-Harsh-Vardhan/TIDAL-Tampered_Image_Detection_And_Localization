...
| 3 | Download CASIA Splicing Detection + Localization via Kaggle API | 02 |
| 4 | Discover image–mask pairs dynamically with dimension validation | 02 |
...
| 16 | Implement threshold-aware early stopping on val Pixel-F1 | 04 |
...
| 25 | Generate Grad-CAM heatmaps | 07 |
| 26 | Create diagnostic overlays (TP/FP/FN) | 07 |
| 27 | Run failure case analysis | 07 |
| 28 | Save `results_summary_v5.json` | 08 |

**Decision gate:** Phase 1 is complete when all 28 tasks are done, the notebook runs end-to-end, and test metrics are reported.
...
| 3 | Plot ROC curve | 07 |
| 4 | Actively use W&B tracking (already integrated, just enable flag) | 09 |
| 5 | (Optional) Add ELA as 4th input channel (`in_channels=4`) | 03 |
| 6 | (Optional) Add dual-task classification head | 03 |

**Note:** W&B support code is already present in the MVP notebook (guarded behind `USE_WANDB = False`). Phase 2 refers to actively enabling and using W&B for run comparison, not writing new code.
...
| 10 | Pixel-level AUC-ROC metric | 05 | No |
