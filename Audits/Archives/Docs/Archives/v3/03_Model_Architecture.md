...
The **same threshold** selected on the validation set is used for both pixel-level binarization and image-level detection. This is intentional: a single operating point keeps the system simple and avoidable a second sweep.
...
ELA highlights regions with inconsistent compression artifacts — a signal that may complement RGB for splicing detection.
...
If ELA is enabled, the dataset class concatenates it as a 4th channel:
...
**ELA rationale (from research literature):** Image tampering detection surveys identify Error Level Analysis as one of the fundamental forensic techniques for detecting JPEG-based manipulations. Re-saving at a fixed quality level causes different compression artifacts in tampered vs. authentic regions, producing visible differences in the ELA map. This is complementary to RGB features and particularly effective for splicing detection.
...
**Status:** Phase 3 experimental only. Not part of MVP or Phase 2.
