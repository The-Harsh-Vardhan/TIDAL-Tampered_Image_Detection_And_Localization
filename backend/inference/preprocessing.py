...
ELA_MEAN = torch.tensor([0.3021,0.2938,0.2847,0.2156,0.2089,0.2012,0.1043,0.1005,0.0963], dtype=torch.float32)
ELA_STD = torch.tensor([0.2187,0.2134,0.2098,0.1876,0.1831,0.1799,0.1234,0.1198,0.1172], dtype=torch.float32)
...
def compute_multi_quality_rgb_ela(image: Image.Image, qualities=None, size: int = DEFAULT_IMAGE_SIZE) -> np.ndarray:
    if qualities is None: qualities = ELA_QUALITIES
    channels = [compute_ela_rgb(image, q, size) for q in qualities]
    return np.concatenate(channels, axis=-1)

def preprocess_image(image: Image.Image, size=DEFAULT_IMAGE_SIZE, qualities=None, mean=None, std=None) -> torch.Tensor:
    if mean is None: mean = ELA_MEAN
    if std is None: std = ELA_STD
