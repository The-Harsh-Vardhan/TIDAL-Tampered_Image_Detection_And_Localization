#!/usr/bin/env python
# Auto-generated from: vR.P.11 Image Detection and Localisation.ipynb
...
SEED = 42
IMAGE_SIZE = 512
BATCH_SIZE = 8  # Halved from P.3's 16 (VRAM constraint at 512x512)
GRAD_ACCUM_STEPS = 2  # Effective batch = 8 * 2 = 16 (same as P.3)
ENCODER = 'resnet34'
...
EPOCHS = 50  # Extended from P.3's 25 (higher res needs more epochs)
...
if torch.cuda.is_available():
    ...
    torch.backends.cudnn.benchmark = True  # Auto-tune convolution for 512x512
...
print(f"Batch:    {BATCH_SIZE} (effective={BATCH_SIZE * GRAD_ACCUM_STEPS} with accum={GRAD_ACCUM_STEPS})")
...
def compute_ela_statistics(image_paths, n_samples=500, size=512):
...
print(f'Batch size:    {BATCH_SIZE} (effective={BATCH_SIZE * GRAD_ACCUM_STEPS} with accum={GRAD_ACCUM_STEPS})')
...
# Single optimizer --- all trainable params at same LR (vR.P.11)
...
# ============================================================
# 5.2 Training and Validation Functions (AMP + Gradient Accumulation)
# ============================================================
...
print('Training functions ready (AMP + gradient accumulation).')
...
print(f'{"Version":<10} {"Track":<12} {"Encoder":<12} {"Input":<12} {"Loss":<14} {"Test Acc":<10} '
f'{"Macro F1":<10} {"AUC":<8} {"Tam-F1":<10} {"Pixel-F1":<10} {"IoU":<8}')
print('-' * 116)

# ETASR results (from vR.1.1)
print(f'{"vR.1.1":<10} {"ETASR":<12} {"2-layer CNN":<12} {"ELA 128sq":<12} {"BCE":<14} '
f'{"88.38%":<10} {"0.8805":<10} {"0.9601":<8} {"0.8606":<10} {"N/A":<10} {"N/A":<8}')

# This run
print(f'{VERSION:<10} {"Pretrained":<12} {"ResNet-34":<12} {"ELA 512sq":<12} {"Focal+Dice":<14} '
