#!/usr/bin/env python
# Auto-generated from: vR.P.14 Image Detection and Localisation.ipynb
...
TTA_VIEWS = 4  # Number of TTA views (orig, hflip, vflip, hvflip)
...
print(f"TTA:      {TTA_VIEWS} views (orig, hflip, vflip, hvflip)")
...
# ============================================================
# ============================================================
# 6.1 Test Set Evaluation --- Pixel-Level Metrics (with TTA)
# ============================================================
...
TTA_INVERSE = [
lambda x: x,                              # no-op
lambda x: torch.flip(x, dims=[-1]),       # undo horizontal flip
lambda x: torch.flip(x, dims=[-2]),       # undo vertical flip
lambda x: torch.flip(x, dims=[-2, -1]),   # undo both flips
]

@torch.no_grad()
def predict_with_tta(model, images, device):
    """Predict with TTA: average predictions from 4 geometric views."""
    preds = []
...
def evaluate_test(model, loader, device):
    ...
    for images, masks, labels in tqdm(loader, desc='Test (TTA)', leave=False):
        images = images.to(device, non_blocking=True)

        # TTA prediction
        tta_probs = predict_with_tta(model, images, device)
        all_preds_tta.append(tta_probs.cpu().numpy())

        # No-TTA prediction (for comparison)
        with autocast('cuda'):
            logits = model(images)
        no_tta_probs = torch.sigmoid(logits.float())
        all_preds_no_tta.append(no_tta_probs.cpu().numpy())

        all_masks.append(masks.numpy())
        all_labels.extend(labels.numpy())

    return (np.concatenate(all_preds_tta), np.concatenate(all_preds_no_tta),
    np.concatenate(all_masks), np.array(all_labels))

preds_tta, preds_no_tta, masks_arr, labels_arr = evaluate_test(model, test_loader, DEVICE)

def compute_pixel_metrics(preds, masks, label=''):
    """Compute pixel-level metrics for a set of predictions."""
    ...
    p_f1 = (2 * tp) / (2 * tp + fp + fn + eps)
    p_iou = tp / (tp + fp + fn + eps)
    p_prec = tp / (tp + fp + eps)
    p_rec = tp / (tp + fn + eps)
    # AUC with subsampling for large pixel counts
    n_total = pred_flat.shape[0]
    if n_total > 5_000_000:
        rng = np.random.RandomState(42)
        idx = rng.choice(n_total, 5_000_000, replace=False)
        p_auc = roc_auc_score(mask_flat[idx], preds.flatten()[idx])
    else:
        p_auc = roc_auc_score(mask_flat, preds.flatten())
    return p_f1, p_iou, p_prec, p_rec, p_auc

# --- Compute both ---
pixel_f1, pixel_iou, pixel_precision, pixel_recall, pixel_auc = \
compute_pixel_metrics(preds_tta, masks_arr, 'TTA')
f1_no, iou_no, prec_no, rec_no, auc_no = \
compute_pixel_metrics(preds_no_tta, masks_arr, 'No TTA')

print('Pixel-Level Metrics Comparison:')
print(f'{"Metric":<20} {"No TTA":<12} {"With TTA":<12} {"Delta":<10}')
print('-' * 54)
print(f'{"Pixel F1":<20} {f1_no:<12.4f} {pixel_f1:<12.4f} {pixel_f1-f1_no:+.4f}')
print(f'{"Pixel IoU":<20} {iou_no:<12.4f} {pixel_iou:<12.4f} {pixel_iou-iou_no:+.4f}')
print(f'{"Pixel Precision":<20} {prec_no:<12.4f} {pixel_precision:<12.4f} {pixel_precision-prec_no:+.4f}')
print(f'{"Pixel Recall":<20} {rec_no:<12.4f} {pixel_recall:<12.4f} {pixel_recall-rec_no:+.4f}')
print(f'{"Pixel AUC":<20} {auc_no:<12.4f} {pixel_auc:<12.4f} {pixel_auc-auc_no:+.4f}')

# --- Alias variables for downstream cells ---
test_probs = preds_tta   # used by cells 6.2, 7.2, etc.
...
print(f'{"Version":<10} {"Track":<12} {"Encoder":<12} {"Input":<10} {"TTA":<6} '
f'{"Test Acc":<10} {"Pixel-F1":<10} {"IoU":<8}')
print('-' * 86)

print(f'{"vR.P.3":<10} {"Pretrained":<12} {"ResNet-34":<12} {"ELA 384sq":<10} {"No":<6} '
f'{"86.79%":<10} {"0.6920":<10} {"0.5291":<8}')

print(f'{"vR.P.10":<10} {"Pretrained":<12} {"ResNet-34":<12} {"ELA 384sq":<10} {"No":<6} '
f'{"87.32%":<10} {"0.7277":<10} {"0.5719":<8}')

# This run (with TTA)
print(f'{VERSION:<10} {"Pretrained":<12} {"ResNet-34":<12} {"ELA 384sq":<10} {"4-view":<6} '
f'{cls_accuracy*100:.2f}{"%-":<7} {pixel_f1:<10.4f} {pixel_iou:<8.4f}')
print()

# Full metrics
print(f'Pixel-Level Metrics (with TTA):')
