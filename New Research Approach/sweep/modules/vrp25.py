#!/usr/bin/env python
# Auto-generated from: vR.P.25 Image Detection and Localisation.ipynb
...
VERSION = 'vR.P.25'
CHANGE = 'Edge Supervision Loss (BCE + Dice + Edge BCE with Sobel boundaries)'
...
print(f'Edge Loss Weight: {EDGE_LOSS_WEIGHT}')
...
# ============================================================
# ============================================================
# 5.1 Loss, Optimizer, Scheduler (CHANGED: + Edge Supervision)
# ============================================================
# Loss = BCE + Dice + edge_weight * Edge_BCE
# Edge ground truth = Sobel edges of the binary GT mask

bce_loss = smp.losses.SoftBCEWithLogitsLoss()
dice_loss = smp.losses.DiceLoss(mode='binary', from_logits=True)
edge_bce_loss = nn.BCEWithLogitsLoss()

# Sobel edge extraction on GPU
...
def compute_edge_map(mask):
    """Extract binary edge map from mask using Sobel filters."""
    # mask: (B, 1, H, W)
    edge_x = F.conv2d(mask, sobel_x, padding=1)
    edge_y = F.conv2d(mask, sobel_y, padding=1)
    edge = torch.sqrt(edge_x ** 2 + edge_y ** 2 + 1e-8)
    edge = (edge > 0.1).float()  # binary edge map
    return edge


def combined_loss_with_edge(logits, targets):
    """BCE + Dice + edge-weighted BCE."""
    loss_bce = bce_loss(logits, targets)
    loss_dice = dice_loss(logits, targets)

    # Edge supervision
    edge_gt = compute_edge_map(targets)
    edge_pred_logits = logits  # use same logits, edge loss focuses on edge pixels
    loss_edge = edge_bce_loss(edge_pred_logits * edge_gt, targets * edge_gt)

    return loss_bce + loss_dice + EDGE_LOSS_WEIGHT * loss_edge
...
scaler = GradScaler()
print(f'Loss: BCE + Dice + {EDGE_LOSS_WEIGHT} * Edge_BCE (Sobel)')
