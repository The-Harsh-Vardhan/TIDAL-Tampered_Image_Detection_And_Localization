#!/usr/bin/env python
# Auto-generated from: vR.P.23 Image Detection and Localisation.ipynb
...
VERSION = 'vR.P.23'
CHANGE = 'Chrominance Channel Analysis (YCbCr Y/Cb/Cr input)'
...
# ============================================================
# ============================================================
# 3.1 PyTorch Dataset and Transforms (CHANGED: YCbCr Chrominance)
# ============================================================

def compute_ycbcr_channels(image_path, size=384):
    """Convert image to YCbCr color space, return (Y, Cb, Cr) channels."""
    img = Image.open(image_path).convert('YCbCr')
    img = img.resize((size, size), Image.BILINEAR)
    return np.array(img)  # (H, W, 3) = (Y, Cb, Cr)


def compute_ycbcr_statistics(image_paths, n_samples=500):
    indices = np.random.choice(len(image_paths), min(n_samples, len(image_paths)), replace=False)
    all_pixels = []
    for idx in indices:
        try:
            img = compute_ycbcr_channels(image_paths[idx])
...
class CASIASegmentationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        try:
            img = compute_ycbcr_channels(self.image_paths[idx], size=self.img_size)
        except Exception:
...
ELA_MEAN, ELA_STD = compute_ycbcr_statistics(train_paths, n_samples=500)
