#!/usr/bin/env python
# Auto-generated from: vR.P.21 Image Detection and Localisation.ipynb
...
VERSION = 'vR.P.21'
CHANGE = 'ELA Residual Learning (Laplacian high-pass filter on ELA)'
...
NUM_WORKERS = 0
ELA_QUALITY = 90
INPUT_TYPE = 'ELA Laplacian Residual'
...
# ============================================================
# ============================================================
# 3.1 PyTorch Dataset and Transforms (CHANGED: ELA Laplacian Residual)
# ============================================================
# Apply Laplacian filter per-channel to ELA image to extract high-frequency
# residuals that emphasize manipulation boundary artifacts.

def compute_ela_laplacian_residual(image_path, quality=90, size=384):
    """Compute ELA then apply Laplacian high-pass per channel."""
    original = Image.open(image_path).convert('RGB')
    buffer = BytesIO()
    original.save(buffer, 'JPEG', quality=quality)
    buffer.seek(0)
    ...
    channels = []
    for c in range(3):
        lap = cv2.Laplacian(ela_np[..., c], cv2.CV_32F, ksize=3)
        # Take absolute value and normalize to [0, 255]
        lap = np.abs(lap)
        if lap.max() > 0:
            lap = lap / lap.max() * 255.0
        channels.append(lap)
...
def compute_ela_lap_statistics(image_paths, n_samples=500):
    indices = np.random.choice(len(image_paths), min(n_samples, len(image_paths)), replace=False)
    all_pixels = []
    for idx in indices:
        try:
            img = compute_ela_laplacian_residual(image_paths[idx])
...
class CASIASegmentationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        try:
            img = compute_ela_laplacian_residual(
...
ELA_MEAN, ELA_STD = compute_ela_lap_statistics(train_paths, n_samples=500)
