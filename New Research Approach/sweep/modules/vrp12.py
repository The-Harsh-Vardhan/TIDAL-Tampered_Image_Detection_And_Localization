#!/usr/bin/env python
# Auto-generated from: vR.P.12 Image Detection and Localisation.ipynb
...
# [SHELL] !pip install -q segmentation-models-pytorch albumentations
...
EPOCHS = 50  # Extended from P.3's 25 (augmented data needs more epochs)
PATIENCE = 10  # Extended from P.3's 7
NUM_WORKERS = 4  # Parallel data loading (optimized)
...
print(f"Augment:  Albumentations (6 transforms, training only)")
...
# ============================================================
# ============================================================
# 3.1 PyTorch Dataset and Transforms (ELA input + Augmentation)
# ============================================================
...
class CASIASegmentationDataset(Dataset):
    """CASIA v2.0 dataset for tampered region segmentation --- ELA input + augmentation."""

    def __init__(self, image_paths, labels, ela_mean, ela_std,
    mask_size=384, ela_quality=90, transform=None):
        self.image_paths = image_paths
        self.labels = labels
        self.mask_size = mask_size
        self.ela_quality = ela_quality
        self.transform = transform  # Albumentations transform (training only)
    ...
    def __getitem__(self, idx):
        ...
        if self.transform is not None:
            ela_np = np.array(ela)  # PIL -> numpy HWC uint8 for Albumentations
            augmented = self.transform(image=ela_np, mask=mask)
            ela = augmented['image']   # numpy HWC uint8
            mask = augmented['mask']   # numpy HW float32

        # to_tensor handles both PIL Image and numpy HWC uint8 -> CHW float [0,1]
...
print(f'Dataset class ready (ELA input, Q={ELA_QUALITY}, augmentation support).')
...
# ============================================================
# ============================================================
# 3.2 Data Splitting (70/15/15 Stratified) + Augmentation + ELA Stats
# ============================================================
...
# Safe augmentations that preserve ELA forensic signal
...
mask_size=IMAGE_SIZE, ela_quality=ELA_QUALITY, transform=train_transform)
val_dataset = CASIASegmentationDataset(
val_paths, val_labels, ela_mean=ELA_MEAN, ela_std=ELA_STD,
mask_size=IMAGE_SIZE, ela_quality=ELA_QUALITY, transform=None)
...
print(f'Augmentation:  Training=YES, Val/Test=NO')
...
# Single optimizer --- all trainable params at same LR (vR.P.12)
...
print(f'{"Version":<10} {"Track":<12} {"Encoder":<12} {"Input":<10} {"Loss":<14} {"Augment":<10} '
f'{"Test Acc":<10} {"Macro F1":<10} {"AUC":<8} {"Pixel-F1":<10} {"IoU":<8}')
print('-' * 114)

# ETASR results (from vR.1.1)
print(f'{"vR.1.1":<10} {"ETASR":<12} {"2-layer CNN":<12} {"ELA 128sq":<10} {"BCE":<14} {"None":<10} '
f'{"88.38%":<10} {"0.8805":<10} {"0.9601":<8} {"N/A":<10} {"N/A":<8}')

# P.3 baseline
print(f'{"vR.P.3":<10} {"Pretrained":<12} {"ResNet-34":<12} {"ELA 384sq":<10} {"BCE+Dice":<14} {"None":<10} '
f'{"---":<10} {"---":<10} {"---":<8} {"0.6920":<10} {"---":<8}')

# This run
print(f'{VERSION:<10} {"Pretrained":<12} {"ResNet-34":<12} {"ELA 384sq":<10} {"Focal+Dice":<14} {"Albu 6x":<10} '
...
'augmentation': 'Albumentations (HFlip, VFlip, Rotate90, ShiftScaleRotate, GaussBlur, BrightContrast)',
'seed': SEED,
}
}, model_filename)
