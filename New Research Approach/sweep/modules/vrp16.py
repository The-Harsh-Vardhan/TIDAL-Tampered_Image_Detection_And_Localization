#!/usr/bin/env python
# Auto-generated from: vR.P.16 Image Detection and Localisation.ipynb
...
# ============================================================
VERSION = 'vR.P.16'
CHANGE = 'DCT spatial feature maps (AC energy / DC / HF energy)'
SEED = 42
IMAGE_SIZE = 384
BATCH_SIZE = 16
ENCODER = 'resnet34'
ENCODER_WEIGHTS = 'imagenet'
IN_CHANNELS = 3  # DCT spatial map has 3 channels (AC energy, DC, HF energy)
NUM_CLASSES = 1
...
print(f"Input:    DCT spatial maps (AC energy / DC / HF energy)")
...
# ============================================================
# ============================================================
# 3.1 PyTorch Dataset and Transforms (CHANGED: DCT Spatial Maps)
# ============================================================
...
def compute_dct_feature_map(image_path, block_size=8, target_size=384):
    ...
    try:
        ...
        # Convert to YCbCr and extract luminance
        ...
        for i in range(n_blocks_h):
            for j in range(n_blocks_w):
                ...
                # DC coefficient (position [0,0])
                dc_value[i, j] = dct_block[0, 0]

                # AC energy (all coefficients except DC)
                ac = dct_block.copy()
                ac[0, 0] = 0
                ac_energy[i, j] = np.sum(ac ** 2)

                # HF energy (bottom-right 4x4 quadrant)
        ...
        # Stack and resize to target size
        dct_map = np.stack([ac_norm, dc_norm, hf_norm], axis=-1)  # (n_blocks_h, n_blocks_w, 3)
...
def compute_dct_statistics(image_paths, n_samples=500, target_size=384):
    """Compute per-channel mean and std of DCT feature maps from training set."""
    ...
    for idx in tqdm(sample_idx, desc='DCT stats', leave=False):
        ...
        arr = dct_map.astype(np.float64) / 255.0  # [0, 1]
...
print('DCT feature extraction functions ready.')
...
class CASIASegmentationDataset(Dataset):
    """CASIA v2.0 dataset for tampered region segmentation --- DCT spatial map input."""

    def __init__(self, image_paths, labels, dct_mean, dct_std, mask_size=384):
        self.image_paths = image_paths
        self.labels = labels
        self.mask_size = mask_size
        self.to_tensor = transforms.ToTensor()
        self.normalize = transforms.Normalize(mean=dct_mean, std=dct_std)
    ...
    def __getitem__(self, idx):
        ...
        # Compute DCT spatial feature map (3-channel: AC energy, DC, HF energy)
        dct_map = compute_dct_feature_map(path, target_size=self.mask_size)
        # dct_map is (H, W, 3) uint8

        dct_tensor = self.to_tensor(dct_map)  # (3, H, W), [0, 1]
        dct_tensor = self.normalize(dct_tensor)  # per-channel normalization

        mask = get_gt_mask(path, self.mask_size)
        mask = torch.tensor(mask, dtype=torch.float32).unsqueeze(0)

        return dct_tensor, mask, label

print(f'Dataset class ready (DCT spatial maps, block_size={DCT_BLOCK_SIZE}).')
print(f'DCT stats will be computed after data splitting.')

# ============================================================
# Cell 9
# ============================================================
# ============================================================
# 3.2 Data Splitting (70/15/15 Stratified) + DCT Stats
# ============================================================
...
# Compute DCT normalization statistics from training set
DCT_MEAN, DCT_STD = compute_dct_statistics(
train_paths, n_samples=500, target_size=IMAGE_SIZE)
print(f'DCT normalization statistics (from 500 training samples):')
...
train_paths, train_labels, dct_mean=DCT_MEAN, dct_std=DCT_STD,
mask_size=IMAGE_SIZE)
val_dataset = CASIASegmentationDataset(
val_paths, val_labels, dct_mean=DCT_MEAN, dct_std=DCT_STD,
mask_size=IMAGE_SIZE)
test_dataset = CASIASegmentationDataset(
test_paths, test_labels, dct_mean=DCT_MEAN, dct_std=DCT_STD,
mask_size=IMAGE_SIZE)
...
try:
    # ============================================================
    # 3.3 Sample Visualization (DCT Input)
    # ============================================================
    ...
    channel_names = ['AC Energy', 'DC Value', 'HF Energy']
    
    for row, idx in enumerate(sample_indices):
        dct_tensor, mask, label = train_dataset[idx]
        dct_display = denormalize_dct(dct_tensor)
    
        # Column 0: Combined DCT (RGB-mapped for overview)
        combined = dct_display.permute(1, 2, 0).numpy()
        axes[row, 0].imshow(combined)
        axes[row, 0].set_title(f'DCT Combined ({"Au" if label==0 else "Tp"})', fontsize=10)
        axes[row, 0].axis('off')
    
        # Columns 1-3: Individual DCT channels
        for ch in range(3):
            axes[row, ch+1].imshow(dct_display[ch].numpy(), cmap='viridis', vmin=0, vmax=1)
            axes[row, ch+1].set_title(f'{channel_names[ch]}', fontsize=10)
        ...
        axes[row, 5].set_title('DCT + Mask Overlay', fontsize=10)
        axes[row, 5].axis('off')
    
    plt.suptitle('Sample Images: DCT Combined | AC Energy | DC Value | HF Energy | GT Mask | Overlay', fontsize=14, y=1.01)
    plt.tight_layout()
    plt.show()
...
print(f'LR: {LEARNING_RATE} | Input: DCT spatial maps (block={DCT_BLOCK_SIZE}) | AMP: Enabled')
print(f'{"="*80}')
...
try:
    ...
    def visualize_predictions(model, dataset, indices, device, title='Predictions'):
        """Display DCT Input | GT Mask | Predicted Mask | Overlay for each index."""
        n = len(indices)
        ...
        for row, idx in enumerate(indices):
            ...
            # Denormalize DCT map for display
            img_display = denormalize_dct(img_tensor).permute(1, 2, 0).numpy()
            gt_display = gt_mask.squeeze(0).numpy()
    
            # Col 0: DCT Input
            axes[row, 0].imshow(img_display)
            axes[row, 0].set_title(f'DCT Input ({"Tampered" if label==1 else "Authentic"})', fontsize=11)
            axes[row, 0].axis('off')
...
# This run
print(f'{VERSION:<10} {"Pretrained":<12} {"ResNet-34":<12} {"DCT 384sq":<14} '
...
'learning_rate': LEARNING_RATE, 'dct_block_size': DCT_BLOCK_SIZE,
'input_type': 'DCT', 'epochs_trained': len(history['train_loss']),
'seed': SEED,
}
}, model_filename)
