#!/usr/bin/env python
# Auto-generated from: vR.P.20 Image Detection and Localisation.ipynb
...
VERSION = 'vR.P.20'
CHANGE = 'ELA Magnitude + Chrominance Direction Decomposition'
...
NUM_WORKERS = 0
ELA_QUALITY = 90
INPUT_TYPE = 'ELA Magnitude Decomposition'
...
# ============================================================
# ============================================================
# 3.1 PyTorch Dataset and Transforms (CHANGED: ELA Magnitude Decomposition)
# ============================================================
# Decompose ELA RGB into 3 informative channels:
#   Ch0: Magnitude = sqrt(R^2 + G^2 + B^2) -- total error intensity
#   Ch1: ChromaDir1 = atan2(G-B, R-mean) -- chrominance angle 1
#   Ch2: ChromaDir2 = (R-G) / (magnitude+eps) -- chrominance ratio

def compute_ela_magnitude_decomposition(image_path, quality=90, size=384):
    """Decompose ELA into magnitude + chrominance direction channels."""
    original = Image.open(image_path).convert('RGB')
    buffer = BytesIO()
    original.save(buffer, 'JPEG', quality=quality)
    buffer.seek(0)
    ...
    ela_np = np.array(ela).astype(np.float32)  # (H, W, 3)

    R, G, B = ela_np[..., 0], ela_np[..., 1], ela_np[..., 2]

    # Channel 0: Magnitude (total error intensity)
    magnitude = np.sqrt(R**2 + G**2 + B**2)

    # Channel 1: Chrominance direction 1
    mean_rgb = (R + G + B) / 3.0
    chroma_dir1 = np.arctan2(G - B, R - mean_rgb + 1e-8)
    # Normalize to [0, 255]
    chroma_dir1 = ((chroma_dir1 + np.pi) / (2 * np.pi) * 255.0)

    # Channel 2: Chrominance ratio
    chroma_dir2 = (R - G) / (magnitude + 1e-8)
    chroma_dir2 = ((chroma_dir2 + 1) / 2 * 255.0).clip(0, 255)

    # Normalize magnitude to [0, 255]
    if magnitude.max() > 0:
        magnitude = magnitude / magnitude.max() * 255.0

    result = np.stack([magnitude, chroma_dir1, chroma_dir2], axis=-1)
    return result.astype(np.uint8)


def compute_ela_mag_statistics(image_paths, n_samples=500):
    indices = np.random.choice(len(image_paths), min(n_samples, len(image_paths)), replace=False)
    all_pixels = []
    for idx in indices:
        try:
            img = compute_ela_magnitude_decomposition(image_paths[idx])
...
class CASIASegmentationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        try:
            img = compute_ela_magnitude_decomposition(
            self.image_paths[idx], quality=ELA_QUALITY, size=self.img_size
            )
        except Exception:
...
ELA_MEAN, ELA_STD = compute_ela_mag_statistics(train_paths, n_samples=500)
