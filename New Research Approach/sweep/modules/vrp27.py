#!/usr/bin/env python
# Auto-generated from: vR.P.27 Image Detection and Localisation.ipynb
...
VERSION = 'vR.P.27'
CHANGE = 'JPEG Compression Augmentation (random Q=50-95 at training time)'
...
NUM_WORKERS = 0
ELA_QUALITY = 90
INPUT_TYPE = 'ELA + JPEG Compression Aug'
JPEG_AUG_RANGE = (50, 95)
JPEG_AUG_PROB = 0.5
...
print(f'JPEG Aug Range: Q={JPEG_AUG_RANGE[0]}-{JPEG_AUG_RANGE[1]}, p={JPEG_AUG_PROB}')
...
# ============================================================
# ============================================================
# 3.1 PyTorch Dataset and Transforms (CHANGED: JPEG Compression Aug)
# ============================================================
# At training time, randomly recompress images before computing ELA.
# This teaches the model to handle various compression levels.

def compute_ela_image(image_path, quality=90, size=384, jpeg_aug_q=None):
    """Compute ELA with optional JPEG recompression augmentation."""
    original = Image.open(image_path).convert('RGB')

    # Optional: recompress before ELA (augmentation)
    if jpeg_aug_q is not None:
        buffer = BytesIO()
        original.save(buffer, 'JPEG', quality=jpeg_aug_q)
        buffer.seek(0)
...
def compute_ela_statistics(image_paths, quality=90, n_samples=500):
    indices = np.random.choice(len(image_paths), min(n_samples, len(image_paths)), replace=False)
    all_pixels = []
    for idx in indices:
        try:
            img = compute_ela_image(image_paths[idx], quality)
...
class CASIASegmentationDataset(Dataset):
    ...
    img_size=384, is_train=False, jpeg_aug_range=None, jpeg_aug_prob=0.0):
        ...
        self.jpeg_aug_range = jpeg_aug_range
    ...
    def __getitem__(self, idx):
        # JPEG compression augmentation (training only)
        jpeg_aug_q = None
        if self.is_train and self.jpeg_aug_range and random.random() < self.jpeg_aug_prob:
            jpeg_aug_q = random.randint(self.jpeg_aug_range[0], self.jpeg_aug_range[1])

        try:
            ela = compute_ela_image(
            self.image_paths[idx], quality=ELA_QUALITY,
            size=self.img_size, jpeg_aug_q=jpeg_aug_q
            )
        except Exception:
            ela = Image.new('RGB', (self.img_size, self.img_size))

        ela = np.array(ela).astype(np.float32) / 255.0
        ela = torch.from_numpy(ela).permute(2, 0, 1)
        for c in range(3):
            ela[c] = (ela[c] - self.ela_mean[c]) / self.ela_std[c]
...
ELA_MEAN, ELA_STD = compute_ela_statistics(train_paths, n_samples=500)
