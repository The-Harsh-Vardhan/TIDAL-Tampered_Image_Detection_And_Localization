#!/usr/bin/env python
# Auto-generated from: vR.P.4 Image Detection and Localisation.ipynb
...
# ============================================================
VERSION = 'vR.P.4'
CHANGE = '4-channel input (RGB + ELA grayscale), conv1 unfrozen for 4th channel'
SEED = 42
IMAGE_SIZE = 384
BATCH_SIZE = 16
ENCODER = 'resnet34'
ENCODER_WEIGHTS = 'imagenet'
IN_CHANNELS = 4  # 3 RGB + 1 ELA grayscale
NUM_CLASSES = 1
LEARNING_RATE = 1e-3  # Single LR (decoder + BN + conv1)
ELA_QUALITY = 90  # ELA JPEG recompression quality
EPOCHS = 25
PATIENCE = 7
...
# ImageNet normalization for RGB channels (0-2)
...
print(f"Input:    4ch (RGB + ELA gray, Q={ELA_QUALITY})")
print(f"Channels: {IN_CHANNELS} (3 RGB + 1 ELA grayscale)")
print(f"Encoder:  {ENCODER} ({ENCODER_WEIGHTS}, frozen body + conv1 unfrozen + BN unfrozen)")
...
# ============================================================
# ============================================================
# 3.1 PyTorch Dataset and Transforms (CHANGED: 4-channel RGB+ELA input)
# ============================================================
...
# ---- Compute ELA grayscale normalization statistics ----
print('Computing ELA grayscale normalization statistics from training set sample...')

def compute_ela_gray_statistics(image_paths, n_samples=500, size=384):
    '''Compute mean and std of ELA grayscale maps from a sample of images.'''
    ...
    for idx in tqdm(sample_idx, desc='ELA gray stats', leave=False):
        ela = compute_ela_image(image_paths[idx], quality=ELA_QUALITY)
        ela_resized = ela.resize((size, size), Image.BILINEAR)
        ela_gray = ela_resized.convert('L')
        arr = np.array(ela_gray, dtype=np.float64) / 255.0  # [0, 1]
        pixel_sum += arr.sum()
...
# Placeholders — will be set after data splitting
ELA_GRAY_MEAN = [0.5]
ELA_GRAY_STD = [0.25]

class CASIASegmentationDataset(Dataset):
    '''CASIA v2.0 dataset for tampered region segmentation -- 4ch (RGB + ELA gray) input.'''

    def __init__(self, image_paths, labels, imagenet_mean, imagenet_std,
    ela_gray_mean, ela_gray_std, mask_size=384, ela_quality=90):
        ...
        self.normalize_rgb = transforms.Normalize(mean=imagenet_mean, std=imagenet_std)
        self.normalize_ela = transforms.Normalize(mean=ela_gray_mean, std=ela_gray_std)
    ...
    def __getitem__(self, idx):
        ...
        # Normalize: first 3 channels with ImageNet, 4th with ELA gray stats
...
print(f'Dataset class ready (4ch: RGB + ELA gray, Q={ELA_QUALITY}).')
print(f'ELA gray stats will be computed after data splitting.')

# ============================================================
# Cell 9
# ============================================================
# ============================================================
# 3.2 Data Splitting (70/15/15 Stratified) + ELA Gray Stats
# ============================================================
...
# Compute ELA grayscale normalization statistics from training set
ELA_GRAY_MEAN, ELA_GRAY_STD = compute_ela_gray_statistics(train_paths, n_samples=500, size=IMAGE_SIZE)
print(f'ELA grayscale normalization statistics (from 500 training samples):')
print(f'  Mean: [{ELA_GRAY_MEAN[0]:.4f}]')
print(f'  Std:  [{ELA_GRAY_STD[0]:.4f}]')
print(f'RGB normalization: ImageNet mean={IMAGENET_MEAN}, std={IMAGENET_STD}')

train_dataset = CASIASegmentationDataset(
train_paths, train_labels,
imagenet_mean=IMAGENET_MEAN, imagenet_std=IMAGENET_STD,
ela_gray_mean=ELA_GRAY_MEAN, ela_gray_std=ELA_GRAY_STD,
...
print(f'\nTrain: {len(train_dataset):>6} images  ({sum(1 for l in train_labels if l==0)} Au + {sum(1 for l in train_labels if l==1)} Tp)')
print(f'Val:   {len(val_dataset):>6} images  ({sum(1 for l in val_labels if l==0)} Au + {sum(1 for l in val_labels if l==1)} Tp)')
print(f'Test:  {len(test_dataset):>6} images  ({sum(1 for l in test_labels if l==0)} Au + {sum(1 for l in test_labels if l==1)} Tp)')
...
try:
    # ============================================================
    # 3.3 Sample Visualization (4-Channel Input)
    # ============================================================
    
    def denormalize_rgb(tensor_3ch, mean=IMAGENET_MEAN, std=IMAGENET_STD):
        '''Reverse ImageNet normalization for RGB channels.'''
        t = tensor_3ch.clone()
        for ch in range(3):
            t[ch] = t[ch] * std[ch] + mean[ch]
        return t.clamp(0, 1)
    
    def denormalize_ela_gray(tensor_1ch, mean=ELA_GRAY_MEAN, std=ELA_GRAY_STD):
        '''Reverse ELA grayscale normalization for display.'''
        t = tensor_1ch.clone()
    ...
    for row, idx in enumerate(sample_indices):
        combined_tensor, mask, label = train_dataset[idx]
    
        rgb_display = denormalize_rgb(combined_tensor[:3]).permute(1, 2, 0).numpy()
        ela_display = denormalize_ela_gray(combined_tensor[3:]).squeeze(0).numpy()
    
        # Column 0: RGB input (channels 0-2)
        axes[row, 0].imshow(rgb_display)
        axes[row, 0].set_title(f'RGB Input ({"Au" if label==0 else "Tp"})', fontsize=10)
        axes[row, 0].axis('off')
    
        # Column 1: ELA grayscale (channel 3)
        axes[row, 1].imshow(ela_display, cmap='hot', vmin=0, vmax=1)
        axes[row, 1].set_title('ELA Gray (ch 3)', fontsize=10)
        axes[row, 1].axis('off')
        ...
        axes[row, 3].set_title('RGB + Mask Overlay', fontsize=10)
        axes[row, 3].axis('off')
    
    plt.suptitle('Sample Images: RGB (ch 0-2) | ELA Gray (ch 3) | GT Mask | Overlay', fontsize=14, y=1.01)
    plt.tight_layout()
    plt.show()
...
# ============================================================
# ============================================================
# 4.1 Build Model (4-Channel Input, Conv1 Unfrozen + BN Unfrozen)
# ============================================================
...
# Step 2: Unfreeze conv1 (first conv layer — modified for 4 channels, needs training)
conv1_param_count = 0
for param in model.encoder.conv1.parameters():
    param.requires_grad = True
    conv1_param_count += param.numel()

# Step 3: Unfreeze BatchNorm layers in encoder (domain adaptation)
bn_param_count = 0
...
print(f'Model: UNet + {ENCODER} ({ENCODER_WEIGHTS}) — 4CH INPUT, CONV1 UNFROZEN + BN UNFROZEN')
print(f'Input channels:       {IN_CHANNELS} (3 RGB + 1 ELA gray)')
print(f'Total parameters:     {total_params:>12,}')
print(f'Trainable parameters: {trainable_params:>12,}')
print(f'  Conv1 params:       {conv1_param_count:>12,}  (unfrozen for 4th channel)')
print(f'  Encoder BN params:  {bn_param_count:>12,}  (unfrozen for domain adaptation)')
print(f'  Decoder:            {decoder_trainable:>12,}  (lr={LEARNING_RATE})')
print(f'  Segmentation head:  {seghead_trainable:>12,}  (lr={LEARNING_RATE})')
print(f'Frozen parameters:    {frozen_params:>12,}  (encoder conv/fc weights except conv1)')
...
# Single optimizer — all trainable params at same LR (vR.P.4)
...
print(f'  Includes: decoder + encoder BN + conv1 (4ch)')
...
print(f'LR: {LEARNING_RATE} | Input: 4ch RGB+ELA (Q={ELA_QUALITY}) | AMP: Enabled')
print(f'{"="*80}')
...
try:
    ...
    def visualize_predictions(model, dataset, indices, device, title='Predictions'):
        '''Display RGB | GT Mask | Predicted Mask | Overlay for each index.'''
        n = len(indices)
        ...
        for row, idx in enumerate(indices):
            ...
            # Denormalize RGB channels (0-2) for display
            img_display = denormalize_rgb(img_tensor[:3]).permute(1, 2, 0).numpy()
            gt_display = gt_mask.squeeze(0).numpy()
    
            # Col 0: RGB Input
            axes[row, 0].imshow(img_display)
            axes[row, 0].set_title(f'RGB Input ({"Tampered" if label==1 else "Authentic"})', fontsize=11)
            axes[row, 0].axis('off')
            ...
            # Col 3: Overlay (green=GT, red=predicted)
    ...
    # Select sample images: 4 tampered + 2 authentic from test set
...
# This run
print(f'{VERSION:<10} {"Pretrained":<12} {"ResNet-34":<12} {"4ch 384²":<10} '
...
'input_type': 'RGB+ELA_4ch', 'in_channels': IN_CHANNELS,
