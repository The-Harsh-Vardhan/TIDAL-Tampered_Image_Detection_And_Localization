...
class ResizeDegradationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        h, w = image.shape[:2]
        small_h, small_w = int(h * self.scale_factor), int(w * self.scale_factor)
...
Image tampering detection models are known to degrade under post-processing operations that destroy forensic traces. Compression artifacts, noise injection, and resolution loss can all mask the subtle statistical inconsistencies between tampered and authentic regions. Testing under these controlled degradations provides evidence of model robustness beyond clean-image performance.

Key research findings informing this test suite:
- **JPEG compression** is the most common real-world degradation; social media platforms re-encode at QF 70–85.
- **Gaussian noise** simulates sensor noise and can obscure splicing boundaries.
- **Blur** destroys high-frequency forensic cues that models may rely on.
- **Resize** causes interpolation artifacts that compete with tampering artifacts.
