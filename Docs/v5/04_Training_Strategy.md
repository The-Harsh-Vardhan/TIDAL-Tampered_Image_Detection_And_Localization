...
**Research context:** The `image-detection-with-mask.ipynb` reference notebook uses a similar combination: `0.5 * BCEWithLogitsLoss + 0.5 * DiceLoss` for segmentation, confirming this as a standard approach. Research papers consistently use overlap-based losses for forgery localization tasks. The reference notebook also experiments with Focal Loss for the classification head — a technique that could be adopted in Phase 2 if a dual-task architecture is implemented.
...
| Max epochs | 50 |
| Early stopping patience | 10 (on threshold-aware val Pixel-F1) |
...
4. **Validation:** Run after each epoch, collect validation probabilities once, and sweep thresholds on the validation set.
...
7. **W&B logging (if enabled):** Log `train/loss`, `val/loss`, `val/pixel_f1`, `val/pixel_iou`, `val/best_threshold`, and learning rates per epoch inside the `USE_WANDB` guard.
...
def load_checkpoint(filepath, model, optimizer, scaler):
    ...
    return ckpt['epoch'] + 1, ckpt['best_f1'], ckpt['best_epoch'], ckpt.get('best_threshold', 0.5)
```

**Storage:** `CHECKPOINT_DIR` — Google Drive in Colab when enabled, otherwise the local artifact directory.
...
**Research note:** The `image-detection-with-mask.ipynb` reference notebook uses `CosineAnnealingLR` with `T_max=10`, validating cosine-based scheduling for this task. Research paper P17 (ME-Net) uses polynomial LR decay with SGD.
...
**Reference notebook augmentations:** The `image-detection-with-mask.ipynb` notebook uses `GaussNoise(p=0.25)` and `JpegCompression(quality_lower=50, quality_upper=90, p=0.25)` during training. These validate our Phase 2 augmentation choices. JPEG compression augmentation is particularly relevant because multiple research papers (P1, P4, P7, P13) demonstrate it as a key differentiator for robustness.
