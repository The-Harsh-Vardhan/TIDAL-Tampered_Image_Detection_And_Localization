#!/usr/bin/env python
# Auto-generated from: vR.P.26 Image Detection and Localisation.ipynb
...
VERSION = 'vR.P.26'
CHANGE = 'Dual-Task: Segmentation + Classification with shared encoder'
...
print(f'Classification Loss Weight: {CLS_LOSS_WEIGHT}')
...
# ============================================================
# ============================================================
# 4.1 Build Model (Dual-Task: UNet + Classification Head)
# ============================================================

class DualTaskUNet(nn.Module):
    """UNet with both segmentation decoder and classification head."""
    def __init__(self, encoder_name='resnet34', in_channels=3):
        ...
        # Classification head: GAP on encoder bottleneck -> FC -> sigmoid
        encoder_channels = self.unet.encoder.out_channels[-1]  # 512 for resnet34
    ...
    def forward(self, x):
        ...
        # Classification path (from bottleneck)
        cls_logits = self.cls_head(features[-1])
        return seg_logits, cls_logits.squeeze(-1)


model = DualTaskUNet(encoder_name='resnet34', in_channels=IN_CHANNELS)

# Freeze strategy: body frozen + BN unfrozen
for name, param in model.unet.encoder.named_parameters():
    if 'bn' in name:
        param.requires_grad = True
    else:
        param.requires_grad = False

# Decoder + cls head always trainable
for param in model.unet.decoder.parameters():
    param.requires_grad = True
for param in model.unet.segmentation_head.parameters():
    param.requires_grad = True
...
print(f'Model: Dual-Task UNet + ResNet-34 (frozen body + BN unfrozen)')
print(f'Segmentation: UNet decoder -> 384x384 mask')
print(f'Classification: GAP -> FC(512,1) -> tampered/authentic')
...
# ============================================================
# ============================================================
# 5.1 Loss, Optimizer, Scheduler (CHANGED: Dual-Task Loss)
# ============================================================
# Loss = seg_loss(BCE + Dice) + cls_weight * cls_loss(BCE)
...
def dual_task_loss(seg_logits, cls_logits, masks, labels):
    ...
    return loss_seg + CLS_LOSS_WEIGHT * loss_cls
...
scaler = GradScaler()
print(f'Loss: (BCE + Dice) + {CLS_LOSS_WEIGHT} * BCE_cls')
