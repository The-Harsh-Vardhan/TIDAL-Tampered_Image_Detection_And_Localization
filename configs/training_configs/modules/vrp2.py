#!/usr/bin/env python
# Auto-generated from: vR.P.2 Image Detection and Localisation.ipynb
...
# ============================================================
VERSION = 'vR.P.2'
CHANGE = 'Gradual unfreeze: layer3+layer4 with differential LR (enc=1e-5, dec=1e-3)'
SEED = 42
IMAGE_SIZE = 384
BATCH_SIZE = 16
ENCODER = 'resnet34'
...
NUM_CLASSES = 1
LEARNING_RATE = 1e-3
ENCODER_LR = 1e-5  # 100x lower than decoder LR for gradual adaptation
EPOCHS = 25
PATIENCE = 7
...
print(f"LR:       {LEARNING_RATE} (decoder), {ENCODER_LR} (encoder layer3+4)")
...
# Create DataLoaders (optimized: persistent_workers, drop_last for train)
...
# ============================================================
# ============================================================
# 4.1 Build Model (Partially Unfrozen Encoder)
# ============================================================
...
print(f'Encoder has {len(encoder_children)} children:')
for i, child in enumerate(encoder_children):
    name = child.__class__.__name__
    n_params = sum(p.numel() for p in child.parameters())
    frozen = i < len(encoder_children) - 2
    status = 'FROZEN' if frozen else 'UNFROZEN'
    print(f'  [{i}] {name:20s}  params={n_params:>10,}  [{status}]')

# Unfreeze layer3 (index -2) and layer4 (index -1)
for block in encoder_children[-2:]:
...
# Count encoder-specific trainable params
...
print(f'\nModel: UNet + {ENCODER} ({ENCODER_WEIGHTS}) — PARTIALLY UNFROZEN')
print(f'Total parameters:     {total_params:>12,}')
print(f'Trainable parameters: {trainable_params:>12,}')
print(f'  Encoder (unfrozen): {encoder_trainable:>12,}  (layer3 + layer4, lr={ENCODER_LR})')
...
# Optimizer with differential learning rates (vR.P.2)
# Group 1: Unfrozen encoder layers (layer3 + layer4) — low LR for cautious adaptation
# Group 2: Decoder + segmentation head — standard LR
...
print(f'Optimizer: Adam with 2 param groups')
print(f'  Group 0 (encoder layer3+4): lr={optimizer.param_groups[0]["lr"]}, '
f'params={sum(p.numel() for p in encoder_params):,}')
print(f'  Group 1 (decoder+seg_head): lr={optimizer.param_groups[1]["lr"]}, '
f'params={sum(p.numel() for p in decoder_params):,}')

# Scheduler (applies to both param groups)
...
def validate(model, loader, criterion, device):
    ...
    for images, masks, labels in tqdm(loader, desc='Val', leave=False):
        ...
        # Collect predictions for metrics (cast to float32 for numpy)
...
print(f'Encoder LR: {ENCODER_LR} | Decoder LR: {LEARNING_RATE} | AMP: Enabled')
print(f'{"="*80}')
