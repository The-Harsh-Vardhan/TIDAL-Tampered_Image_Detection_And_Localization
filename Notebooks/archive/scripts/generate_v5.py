"""Generate tamper_detection_v5.ipynb from Docs5 specifications."""
import json
import os

def to_source(text):
    """Convert a text block to notebook source format (list of lines with \\n)."""
    lines = text.split('\n')
    result = []
...
md("""# Tamper Detection v5 — Image Forgery Detection & Localization
...
17. Save & Export Results

**Changelog from v4:**
- Split notebook into 17 clearly defined sections
- Added Explainable AI section (Grad-CAM, diagnostic overlays, failure analysis)
- Added Dataset Validation section
- Updated artifact naming to v5
- Integrated W&B logging throughout (guarded behind `USE_WANDB`)""")

md("## 1. Setup & Environment")

code("""!pip install -q kaggle segmentation-models-pytorch "albumentations>=1.3.1,<2.0" """)
...
md("""## 2. Kaggle Dataset Download

Download the CASIA Splicing Detection + Localization dataset via Kaggle API.
...
code("""# Kaggle API authentication
# Option 1: Colab Secrets (recommended for security)
try:
    ...
    # Option 2: Manual entry (uncomment and fill in)
    # os.environ['KAGGLE_USERNAME'] = 'your_username'
    # os.environ['KAGGLE_KEY'] = 'your_key'
    print('Set KAGGLE_USERNAME and KAGGLE_KEY via Colab Secrets or manually.')
...
# Discover dataset root dynamically (find directory with Image/ and Mask/)
DATASET_ROOT = None
for root, dirs, files in os.walk(DOWNLOAD_DIR):
    if 'Image' in dirs and 'Mask' in dirs:
        DATASET_ROOT = root
...
if DATASET_ROOT is None:
    raise FileNotFoundError(
        'Could not find dataset root with Image/ and Mask/ directories. '
        'Check the download and extraction above.'
    )
...
manifest_path = os.path.join(CHECKPOINT_DIR, 'split_manifest.json')
...
plt.tight_layout()
plt.savefig(os.path.join(CHECKPOINT_DIR, 'f1_vs_threshold.png'), dpi=150, bbox_inches='tight')
plt.show()
...
plt.tight_layout()
plt.savefig(os.path.join(CHECKPOINT_DIR, 'prediction_grid.png'), dpi=150, bbox_inches='tight')
plt.show()
...
if USE_WANDB:
    wandb.log({
        'predictions': wandb.Image(os.path.join(CHECKPOINT_DIR, 'prediction_grid.png')),
    })
    wandb.log({
        'training_curves': wandb.Image(os.path.join(CHECKPOINT_DIR, 'training_curves.png')),
    })
...
plt.tight_layout()
plt.savefig(os.path.join(CHECKPOINT_DIR, 'gradcam_analysis.png'), dpi=150, bbox_inches='tight')
plt.show()
...
plt.tight_layout()
plt.savefig(os.path.join(CHECKPOINT_DIR, 'robustness_chart.png'), dpi=150, bbox_inches='tight')
plt.show()

# W&B logging (guarded)
if USE_WANDB:
    wandb.log({
        'robustness_chart': wandb.Image(os.path.join(CHECKPOINT_DIR, 'robustness_chart.png')),
    })
    for name, data in robustness_results.items():
...
When `USE_WANDB = False` (default), the notebook produces local artifacts on Google Drive:
...
results_path = os.path.join(CHECKPOINT_DIR, 'results_summary_v5.json')
...
if USE_WANDB:
    ...
    print('W&B disabled. All artifacts saved locally to Google Drive.')
...
for f_name in [
    'best_model.pt', 'last_checkpoint.pt', 'split_manifest.json',
    'results_summary_v5.json', 'training_curves.png', 'f1_vs_threshold.png',
